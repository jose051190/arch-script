...
#-------------------------------------
PACOTES_ESSENCIAIS=(wl-clipboard yazi fd ffmpegthumbnailer ffmpeg unzip unrar 7zip jq poppler zoxide imagemagick npm fwupd fzf ttf-nerd-fonts-symbols inter-font noto-fonts ttf-jetbrains-mono-nerd plymouth adobe-source-code-pro-fonts neovim rclone fastfetch htop btop ncdu virt-manager qemu-full dnsmasq edk2-ovmf spice-vdagent firewalld pacman-contrib pacutils expac less ksystemlog rsync sshfs go docker docker-compose cronie)
